...
export const heightStyle: { [k in ButtonHeight]: string } = {
...
export const disabledStyles = 'disabled:opacity-30 disabled:cursor-not-allowed';

export default function BaseButton({ height, color, leftIcon, rightIcon, children, className, ...restProps }: ButtonProps) {
  const buttonClass = useMemo(() => {
    return clsx('flex justify-center items-center text-white', heightStyle[height], colorStyles[color], disabledStyles, className);
