...
export default function DualBanner() {
  const { data: dualBannerList } = useGetBannerList('dual');
        ...
        dualBannerList.map((banner, index) => (
          <Link key={index} href={banner.banner_link}>
            <div className='relative'>
              <Image src={banner.banner_image_url} width={500} height={170} alt={`${banner.banner_name} banner image`} className='object-cover rounded-[10px]' />
