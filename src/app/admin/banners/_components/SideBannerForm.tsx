    ...
    <section>
      ...
      <div className='flex flex-col gap-2'>
        ...
        <div className='flex items-center gap-2'>
          <div className='flex items-center gap-2'>
            <Switch id='is-active-mode' checked={sideBanner?.is_active} onCheckedChange={onToggleBannerStatus} />
            <label htmlFor='is-active-mode' className='text-body-14m text-ui-text-body'>
              {sideBanner?.is_active ? '활성화' : '비활성화'}
            </label>
          </div>

          <Button height={32} color='gray' className='w-fit' onClick={() => onDeleteBanner(sideBanner?.banner_id)} disabled={isDeleting}>
            삭제
          </Button>
        </div>
      </div>
