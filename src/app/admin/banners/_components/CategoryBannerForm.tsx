...
export default function CategoryBannerForm() {
  const { data: sideBannerList, isLoading } = useGetAdminBannerList('category');
  const { mutate: updateBannerStatus, isPending } = useUpdateBannerStatus('category');
  const { mutate: deleteBanner, isPending: isDeleting } = useDeleteBanner('category');
  ...
  const onDeleteBanner = (banner_id: number) => {
  ...
  return (
    <section>
      <h3 className='text-title-24r mb-3'>카테고리 배너</h3>
            ...
            <div key={banner.banner_id} className='flex flex-col gap-2 w-full max-w-[300px]'>
              ...
              <div className='flex justify-between items-center gap-2'>
                <div className='flex items-center gap-2'>
                  <Switch id='is-active-mode' checked={banner.is_active} onCheckedChange={() => onToggleBannerStatus(banner.banner_id, banner.is_active)} />
                  <label htmlFor={`is-active-${banner.banner_id}`} className='text-body-14m'>
                    {banner.is_active ? '활성화' : '비활성화'}
                  </label>
                </div>

                <Button height={32} color='gray' className='w-fit' onClick={() => onDeleteBanner(banner.banner_id)} disabled={isDeleting}>
                  삭제
                </Button>
              </div>
            </div>
